...
public class Hmode_course extends AppCompatActivity {
    ImageView back;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.hmodecourse);
        ...
        panier.clear();
        ItemsProduct.clear();
       // Toast.makeText(getApplicationContext(), "T3det : " + Hproducts.size(), Toast.LENGTH_LONG).show();
        myDialog = new Dialog(this);
        pricetotal=0000;
        Getintent();
        modecourseAdapter = new HModecourseAdapter(ItemsProducts, getApplicationContext());
    ...
    public void Getintent() {
        ...
        if(test){
            Hproducts.addAll(Hproduct.getproductpaniers());
            txtnamelist.setText(nameList);
            GetLocationMobile();
            getListdeprixMS(idList,idPOS);
        }else {
            basket.addAll(HBasket.getBasket());
            ItemsProducts.addAll(HBasket.getItemlist());
                                ...
                                if (jsonObject.getString("arraymodel").equals("[]")){
                                    ...
                                    for (int i = 0; i < jsonArray.length(); i++) {
                                      ...
                                      //  Toast.makeText(getApplicationContext(), "T3det : " + Hproducts.size(), Toast.LENGTH_LONG).show();
                                        ...
                                        ItemsProduct.add(new ItemModeCourse(nameProduct,nameCategory,nameMark,nameModel,unit,quantity,idModel,idProduct,idMark,price));

                                    }
                                    for (int i = 0; i < ItemsProduct.size(); i++) {
                                        for (int j = 0; j < Hproducts.size( ); j++) {

                                            if (Hproducts.get(j).getidModel().equals(ItemsProduct.get(i).getidModel()) && Hproducts.get(j).getquantity().equals(ItemsProduct.get(i).getquantity()) && Hproducts.get(j).getunit().equals(ItemsProduct.get(i).getunit())) {
                                                ItemsProducts.add(ItemsProduct.get(i));
                                            }
                                        }
                                    }
    ...
    public void popupPointOfSale(ArrayList<ItemPos> listpos) {
        ...
        posAdapter = new PosAdapter(Hmode_course.this,listpos);
    ...
    public void gobasket(){
        Intent i = new Intent(getApplicationContext(), HBasket.class);
